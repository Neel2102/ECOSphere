...
const base = makeModel({
  ...
  writable: [
    ...
    'quantity', 'unit', 'co2_amount', 'transaction_date', 'auto_calculated', 'created_by', 'organization_id',
  ],
  order: 'transaction_date DESC, id DESC',
  search: ['reference'],
});

async function listDetailed({ q, department_id, source_type, from, to, organizationId } = {}) {
  const params = [];
  const clauses = [];
  ...
  if (organizationId) { params.push(organizationId); clauses.push(`t.organization_id = $${params.length}`); }
...
async function monthlyTotals(months = 12, organizationId) {
  const params = [months];
  let filter = '';
  if (organizationId) {
    params.push(organizationId);
    filter = ' AND organization_id = $2';
  }
  const { rows } = await query(
    `SELECT TO_CHAR(transaction_date, 'YYYY-MM') AS month,
            ROUND(SUM(co2_amount)::numeric, 2) AS total_co2
     FROM carbon_transactions
     WHERE transaction_date >= date_trunc('month', CURRENT_DATE) - ($1 - 1) * INTERVAL '1 month'${filter}
...
async function totalsByDepartment({ from, to, organizationId } = {}) {
  const params = [];
  const clauses = [];
