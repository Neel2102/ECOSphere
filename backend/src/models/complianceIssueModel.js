...
const base = makeModel({
  ...
  writable: [
    ...
    'owner_id', 'due_date', 'status', 'resolved_at', 'organization_id',
  ],
  order: 'due_date ASC',
  search: ['title'],
});

async function listDetailed({ q, status, severity, department_id, owner_id, organizationId } = {}) {
  const params = [];
  const clauses = [];
  ...
  if (organizationId) { params.push(organizationId); clauses.push(`i.organization_id = $${params.length}`); }
...
async function findNewlyOverdue(organizationId) {
  const { rows } = await query(
    `SELECT i.*, u.full_name AS owner_name
     FROM compliance_issues i
     JOIN users u ON u.id = i.owner_id
     WHERE i.status != 'resolved' AND i.due_date < CURRENT_DATE AND i.overdue_notified = FALSE AND i.organization_id = $1`,
    [organizationId]
  );
  return rows;
}
...
async function openBySeverityPerDepartment(organizationId) {
  const { rows } = await query(
    `SELECT department_id, severity, COUNT(*)::int AS count
     FROM compliance_issues
     WHERE status != 'resolved' AND department_id IS NOT NULL AND organization_id = $1
