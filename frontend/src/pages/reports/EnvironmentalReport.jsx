...
function EnvironmentalReport() {
  ...
  const handleDownload = (format) => {
    const url = reportService.getReportDownloadUrl('environmental', format, {
  ...
  const columns = (report?.columns || []).map((col) => ({
    key: col.key,
    title: col.label,
    render: col.key === 'co2_amount' ? (r) => `${Number(r.co2_amount).toFixed(2)} kg` : undefined,
  }));
